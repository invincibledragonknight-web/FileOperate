...
from deepagents.middleware.skills import SkillsMiddleware as BackendSkillsMiddleware
from deepagents.middleware.subagents import CompiledSubAgent, SubAgent, SubAgentMiddleware
from deepagents.skills.load import normalize_skills_dirs
from deepagents.skills.middleware import SkillsMiddleware as DirectorySkillsMiddleware
...
def create_deep_agent(
    ...
    *,
    ...
    Args:
        ...
        skills_dirs: Optional skills directories to scan for SKILL.md files. Accepts a
            single path, a (path, virtual_root) tuple (virtual_root should start with
            "/"), or a sequence of those. When provided, skills are injected into the
            system prompt for the main agent and subagents. If set, the `skills` backend
            sources are ignored.
    ...
    normalized_skills_dirs = normalize_skills_dirs(skills_dirs) if skills_dirs else []
    if normalized_skills_dirs:
        skills = None
    ...
    if normalized_skills_dirs:
        subagent_middleware.append(DirectorySkillsMiddleware(skills_dirs=normalized_skills_dirs))
    elif skills is not None:
        subagent_middleware.append(BackendSkillsMiddleware(backend=backend, sources=skills))
    ...
    if normalized_skills_dirs:
        deepagent_middleware.append(DirectorySkillsMiddleware(skills_dirs=normalized_skills_dirs))
    elif skills is not None:
        deepagent_middleware.append(BackendSkillsMiddleware(backend=backend, sources=skills))
    deepagent_middleware.extend(
        [
            SubAgentMiddleware(
                default_model=model,
